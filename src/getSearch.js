...
function GetSearch(searchRGB) {
    ...
    while(i<averageRGB.length){
        ...
        diffSearch.EuclideanD =  ((averageRGB[i].r-searchRGB[0])**2)**0.5 +((averageRGB[i].g-searchRGB[1])**2)**0.5 +((averageRGB[i].b-searchRGB[2])**2)**0.5
    ...
    for(let i =0;i<=9;i++){
        ...
        searchImageID.src = process.env.PUBLIC_URL +"/image/"+diffSearchArray[i].index+".png"
    ...
    document.getElementById("status").innerHTML=`搜尋RGB(${searchRGB})耗時${endTime-startTime} ms`
